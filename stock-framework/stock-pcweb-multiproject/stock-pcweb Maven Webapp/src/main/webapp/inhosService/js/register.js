function back(){
	window.location.href="http://localhost:8080/blsb-pcweb/login.html";
}
...
(function(){
	...
	function yzmClick(){
			if(check(telPhone,"telTips")){
				...
				$.ajax({
					url:"http://localhost:8080/blsbWebappBase/employee/sendValidateCode.do",
					type:"POST",
					dataType: 'text',
				    crossDomain: true,
...
function lastStep(phoneNum){
	...
	if(pwd==pwd2){
		$.ajax({
			url :"http://localhost:8080/blsbWebappBase/customer/insertNewCustomer.do",
			contentType:"application/x-www-form-urlencoded; charset=UTF-8",
			type:"POST",
			dateType:"json",
			data:{
